...
void winetest_wait_child_process( HANDLE process )
{
    ...
    if (ret == WAIT_TIMEOUT)
        ...
        winetest_ok( 0, "Could not wait for the child process: %ld le=%lu\n",
                     ret, GetLastError() );
    else
    {
        DWORD exit_code;
        ...
        if (exit_code > 255)
        {
            ...
            printf( "%s:%d:%s unhandled exception %08lx in child process %04lx\n",
                    data->current_file, data->current_line, winetest_elapsed(),
                    exit_code, pid );
            InterlockedIncrement( &failures );
        }
        ...
        {
            printf( "%s:%d:%s %lu failures in child process\n",
                    data->current_file, data->current_line, winetest_elapsed(),
                    exit_code );
            while (exit_code-- > 0)
                InterlockedIncrement(&failures);
...
static int run_test( const char *name )
{
    ...
    if (winetest_debug)
    {
        if (muted_todo_successes || muted_skipped || muted_traces)
            printf( "%04lx:%s:%s Silenced %ld todos, %ld skips and %ld traces.\n",
                    GetCurrentProcessId(), test->name, winetest_elapsed(),
                    muted_todo_successes, muted_skipped, muted_traces);
        printf( "%04lx:%s:%s %ld tests executed (%ld marked as todo, %ld %s), %ld skipped.\n",
                GetCurrentProcessId(), test->name, winetest_elapsed(),
...
static LONG CALLBACK exc_filter( EXCEPTION_POINTERS *ptrs )
{
    ...
    printf( "%04lx:%s:%s unhandled exception %08lx at %p\n",
